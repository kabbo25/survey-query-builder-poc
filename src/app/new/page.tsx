...
import { saveLabel, type SavedLabel } from "@/lib/store";
...
// ========== MAIN PAGE ==========
function extractSummary(root: GroupNode): SavedLabel["summary"] {
  const summary = { ageRange: "-", education: "-", experience: "-", banglaEnglish: "-", computerSkill: "-", minorityGroup: "-", gender: "-" };
  function walk(node: Node) {
    if (node.type === "rule" && node.qkey && node.op && node.value != null) {
      const v = Array.isArray(node.value) ? node.value.join(", ") : String(node.value);
      if (node.qkey === "applicant_dob" || node.qkey.includes("age")) summary.ageRange = v;
      else if (node.qkey.includes("education") || node.qkey === "highest_education_level") summary.education = v;
      else if (node.qkey.includes("experience") || node.qkey.includes("employ")) summary.experience = v;
      else if (node.qkey.includes("bangla") || node.qkey.includes("read_write")) summary.banglaEnglish = v;
      else if (node.qkey.includes("computer") || node.qkey.includes("digital")) summary.computerSkill = v;
      else if (node.qkey.includes("indigenous") || node.qkey.includes("minority")) summary.minorityGroup = v;
      else if (node.qkey.includes("gender")) summary.gender = v;
        ...
        <div className="page-head">
          ...
          <div className="head-actions">
            ...
            <button className="btn" onClick={() => {
              saveLabel({ id: crypto.randomUUID(), nameEn: nameEn || "(untitled)", nameBn, type: labelType, status: "draft", query: root, createdAt: new Date().toISOString(), summary: extractSummary(root) });
              router.push("/");
